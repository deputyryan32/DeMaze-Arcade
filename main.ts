namespace SpriteKind {
    ...
    export const EndChest4 = SpriteKind.create();
}
...
info.onScore(1, function (){
    sprites.onOverlap(SpriteKind.Player, SpriteKind.EndChest, function (sprite: Sprite, otherSprit2: Sprite) {
        ...
        tiles.placeOnTile(myPlayer, tiles.getTileLocation(0, 16))
        info.setScore(0)
        info.stopCountdown
...
info.onScore(0, function () {
    sprites.onOverlap(SpriteKind.Player, SpriteKind.EndChest3, function (sprite: Sprite, otherSprite: Sprite) {
        ...
        tiles.placeOnTile(myPlayer, tiles.getTileLocation(3, 20))
        info.setScore(0)
        info.stopCountdown
        info.startCountdown(80.1)
        let chest4 = sprites.create(assets.image`chest4`, SpriteKind.EndChest4)
        tiles.placeOnRandomTile(chest4, assets.tile`myTileBad4`)
        // Pizza2
        let pizza2 = sprites.create(assets.image`pizza`, SpriteKind.Collectable)
        tiles.placeOnRandomTile(pizza2, assets.tile`myTileBad4`)

        let pizza3 = sprites.create(assets.image`pizza`, SpriteKind.Collectable)
        tiles.placeOnRandomTile(pizza3, assets.tile`myTileBad4`)

        // Pie
        let pie = sprites.create(assets.image`pie`, SpriteKind.Collectable)
        tiles.placeOnRandomTile(pie, assets.tile`myTileBad4`)
        let pie2 = sprites.create(assets.image`pie`, SpriteKind.Collectable)
        tiles.placeOnRandomTile(pie2, assets.tile`myTileBad4`)

        game.splash("How to Win.", "Find 2 pizzas and Pies and reach the chest!")
    })
    sprites.onOverlap(SpriteKind.Player, SpriteKind.EndChest4, function (sprite: Sprite, otherSprite: Sprite) {
        sprites.destroy(otherSprite);
